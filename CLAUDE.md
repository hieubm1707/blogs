...
This is a personal CV and blog site: VitePress with a custom theme, content managed by Decap CMS, deployed free on Netlify. The site is **bilingual: English (default, at `/`) and Vietnamese (under `/vi/`)**, for both the CV and the blog. CMS labels are in English and the docs (`README.md`) are in Vietnamese. The user talks in Vietnamese; reply in Vietnamese.
...
- **Routing happens in `Layout.vue`**, based on frontmatter and path:
  ...
  `index.md`, `vi/index.md`, `blogs.md` and `vi/blogs.md` set `layout: false`.
- **The CV is data-driven.** All CV content lives in the YAML frontmatter of `index.md` (English) and `vi/index.md` (Vietnamese, same keys and structure), and `CvProfile.vue` renders it through `useData().frontmatter`. Keys: `name, headline, bio, status, location, avatar, email, github, linkedin, cvLink, highlights[], experiences[], skillGroups[], projects[], education[], certifications[]`. To change CV content, edit the YAML, not the Vue file. `CvProfile.vue` has no placeholder fallbacks; each section hides itself when its key is missing. Only put facts the user has provided into the CV; never invent roles, numbers or certifications. Never publish the employer's business metrics (user counts, MAU, revenue) because of confidentiality; technical figures the user gave (table size, latency) are fine. If you add a key, update `CvProfile.vue` and both CV files. Any content change to one CV must be mirrored in the other; keep tech, company, project names and job titles in English in both.
...
- **i18n**:
  ...
  - `Layout.vue` computes the link to the other language: the CV (`/` ↔ `/vi/`, via `locales.<lang>.home`), the blog list, and posts that have a translation (matched by slug). `LocaleSwitcher.vue` renders it as two flags (inline SVG in `Flag.vue`, because flag emoji don't render on Windows): the current one is ringed, the other is a link, or greyed out when a post has no translation. Section headings and buttons of `CvProfile.vue` also come from `messages`.
- **RSS**: `.vitepress/genFeed.ts` runs as the `buildEnd` hook and writes one feed per locale: `feed.rss` and `vi/feed.rss`.
- **Styling**: Tailwind 3 through `postcss.config.js`. Its `content` glob only scans `./.vitepress/theme/**/*.vue`, so classes used anywhere else get purged. The accent color is emerald.
- **Light/dark theme**: light by default. Tailwind uses `darkMode: 'class'`, so `dark:` classes apply when `<html>` has the `dark` class. VitePress' own dark mode is off (`appearance: false`) because it follows the OS preference. Instead, `theme/theme.ts` provides an inline head script (added in `config.ts`) that sets `dark` before first paint when `localStorage.theme === 'dark'`, and `ThemeToggle.vue` in the header toggles the class and stores the choice. Never use `@media (prefers-color-scheme: dark)` in CSS; scope dark styles with `.dark` instead.
