...
- **i18n**:
  ...
  - `Layout.vue` shows a language switcher on the CV (`/` ↔ `/vi/`, via `locales.<lang>.home`), on the blog list, and on posts that have a translation (matched by slug). Section headings and buttons of `CvProfile.vue` also come from `messages`.
